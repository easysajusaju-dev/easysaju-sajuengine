...
interface EngineResponse {
  ...
  result?: {
    ...
    seun?: { years: number[]; ganji: string[] };
    wolun?: { year: number; months: number[]; ganji: string[] };
  };
  error?: string;
}
...
function getNabeum(ganji: string) {
  return NABEUM[ganji] || "";
}
      ...
      <div className="w-full max-w-md bg-white shadow-2xl min-h-screen flex flex-col relative">
        ...
        <header className="bg-[#3F51B5] text-white shadow-md z-20">
          ...
          <div className="flex text-sm font-medium text-center">
            ...
            <div className="flex-1 py-3 cursor-pointer text-indigo-200">
              저장 목록
            </div>
            <div className="flex-1 py-3 cursor-pointer text-indigo-200">
              도움 말
        ...
        {viewMode === "result" && engineResult && debugData && (
          <main className="flex-1 overflow-y-auto bg-white pb-20">
            ...
            <section className="border-b-2 border-gray-300">
              ...
              <div className="grid grid-cols-4 text-center bg-gray-300 text-gray-800 text-sm font-bold border-b border-gray-400">
                {["hour", "day", "month", "year"].map((col) => (
                  <div
                    key={col}
                    className="py-1 border-r border-gray-400 last:border-r-0"
                  >
                    {col === "hour"
                      ? "시주"
                      : col === "day"
              ...
              <div className="grid grid-cols-4 text-center text-xs font-bold h-6 items-center bg-[#FFF9C4] border-b border-gray-400 text-red-600">
                {["hour", "day", "month", "year"].map((col) => (
                  <div
                    key={col}
                    className="border-r border-gray-400 last:border-none"
                  >
              ...
              <div className="grid grid-cols-4 text-center text-sm border-b border-gray-300 bg-white py-1">
                {["hour", "day", "month", "year"].map((col) => {
                  ...
                  return (
                    <div
                      key={col}
                      className="border-r border-gray-300 last:border-none flex flex-col justify-center"
                    >
                      {chars.map((char, idx) => {
                        const label =
                          chars.length === 2
              ...
              <div className="grid grid-cols-4 text-center text-xs font-bold h-8 items-center bg-[#FFF9C4] border-t border-gray-400">
                {["hour", "day", "month", "year"].map((col) => (
                  <div
                    key={col}
                    className="border-r border-gray-400 last:border-none flex flex-col justify-center"
                  >
                    {getColumnData(col as any)?.relations}
                  </div>
                ))}
              ...
              {/* 공망/귀인 (임시 하드코딩 그대로 유지) */}
              <div className="text-center text-sm py-1 bg-white border-t border-black">
                공망: [년]{engineResult.ganji.year} [일]
                {engineResult.ganji.day}, 천을귀인: 子申, 월령: 庚
              </div>
            </section>
                ...
                <div className="min-w-[350px]">
                  ...
                  <div className="grid grid-cols-10 bg-white border-b border-gray-300 text-sm text-center font-medium">
                    {Array.from({ length: 10 }).map((_, i) => (
                      <div
                        key={i}
                        className="py-1 border-r border-gray-300 last:border-none"
                      >
                  ...
                  {/* 대운 간지: 기존 만세력 서버 배열 그대로 사용 (간지는 이미 맞음) */}
            ...
            {engineResult.seun && (
              <section className="mt-1 border-t-4 border-gray-300">
                ...
                <div className="grid grid-cols-5 sm:grid-cols-10 border-b border-gray-300">
                  {engineResult.seun.years.slice(0, 10).map((year, idx) => {
                    const ganji = engineResult.seun?.ganji[idx] || "--";
            ...
            {/* 월운 – 엔진 계산값 사용       */}
            {/* ============================ */}
            {engineResult.wolun && engineResult.wolun.months.length > 0 && (
              <section className="mt-1 border-t-4 border-gray-300">
                <div className="bg-white text-center py-1.5 font-bold text-base border-b border-gray-400">
                  월운 ({engineResult.wolun.year}년)
                </div>

                <div className="grid grid-cols-4 sm:grid-cols-6 border-b border-gray-300">
                  {engineResult.wolun.months.map((m, idx) => {
                    const ganji = engineResult.wolun?.ganji[idx] || "--";
                    return (
                      <div
                        key={m}
                        className="flex flex-col items-center py-2 border-r border-gray-300 bg-white"
                      >
                        <span className="text-sm mb-1 font-bold text-black">
                          {m}월
                        </span>
        ...
        {/* 에러 메시지 (있으면) */}
        {error && (
          <div className="fixed bottom-16 left-0 right-0 mx-auto max-w-md px-4">
            <div className="bg-red-100 text-red-700 text-sm px-3 py-2 rounded shadow">
              {error}
            </div>
          </div>
        )}
