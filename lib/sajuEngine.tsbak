...
object SajuService {
    ...
    // 천간 오행 (영문 키: tenGod, branch 십성에서 같이 사용)
    ...
    private fun tenGod(dayStem: String, stem: String): String {
        val ed = elemOfStem(dayStem)      // 일간 오행
        val eo = elemOfStem(stem)         // 비교 대상 오행
        val same = isYangStem(dayStem) == isYangStem(stem)  // 음양 같냐?
        ...
        return when {
            ...
            // 나를 생하는 오행(인성) : same=편인, diff=정인
            eo == CYCLE[(idx(ed) + 4) % 5] ->
                if (same) "편인" else "정인"

            // 내가 극하는 오행(재성) : same=편재, diff=정재
            eo == CONTROL[ed] ->
                if (same) "편재" else "정재"

            // 나를 극하는 오행(관성) : same=편관, diff=정관
            CONTROL[eo] == ed ->
                if (same) "편관" else "정관"
    ...
    // 지지 기준 십성 (프론트 getSibsung(dayStem, branch, true) 대응)
    ...
    private fun getBranchRelations(yearBr: String, monthBr: String, dayBr: String, hourBr: String): BranchRelations {
        val branches = mapOf(
            "year" to yearBr,
            "month" to monthBr,
            "day" to dayBr,
        ...
        fun push(list: MutableList<BranchRelationItem>, kind: String, a: String, b: String) {
            val ba = branches[a] ?: ""
            val bb = branches[b] ?: ""
            list.add(BranchRelationItem(from = a, to = b, branches = ba + bb, kind = kind))
        }
        ...
        return BranchRelations(hyung = hyung, chung = chung, pa = pa, hap = hap)
    }

    // =============== 월운 계산 (프론트 calcWolun 동일) ===============

    private val MONTH_BRANCH_ORDER = listOf("寅","卯","辰","巳","午","未","申","酉","戌","亥","子","丑")
    ...
    fun getSaju(
        ...
        val forward = (isMale && isYangStem(yearStem)) || (!isMale && !isYangStem(yearStem))
        ...
        val diffHours = diffDays * 24.0  // debug용
        ...
        val relations = getBranchRelations(yearBranch, monthBranch, dayBranch, hourBranch)
